...
public abstract class StringUtils {
    ...
    public static float camelSearch(String origin, String filterText) {
        if (filterText.length() > origin.length()) {
            return 0;
        }
        char[] filters = filterText.toLowerCase().toCharArray();
        ...
        float rate=1.1f;
        float score = 0; // 得分
        for (int k = 0; i < filters.length && k < origins.length; k++) {
            if (filters[i] == origins[k] || filters[i] == Character.toLowerCase(origins[k])) {
                score+=increaseScore;
                increaseScore*=rate;// 连续得分累加
                i++;
            } else { // 找到下一个非小写母
                for (; k < origins.length && origins[k] < 'a'; k++) {
                }
                increaseScore=10f;// 累加分复原
            }
        }
        //
        if(i!=filters.length)
            return 0;// 匹配失败
        return score*filterText.length()/origin.length(); //  匹配得分
    }
}
